...
import {
  ...
  FileAccessData, DependencyData, EnvironmentItemData
} from '../base/structure';
...
interface DiffModuleData {
  modules_added: DependencyData[];
  modules_removed: DependencyData[];
  modules_replaced: DependencyData[][];
}
...
class DiffInfoWidget extends Widget {
  ...
  static dependency_url(trial1: string, trial2: string) {
    return ("http://127.0.0.1:5000/diff/"
  ...
  load(trial1: string, trial2: string) {
    ...
    json("Info", sub, DiffInfoWidget.url(trial1, trial2), (data: DiffInfoData) => {
      ...
      this.loadDependencies(trial1, trial2);
  ...
  createFold(parent: d3_Selection<d3_BaseType, {}, HTMLElement, any>, title: string) {
  ...
  loadDependencies(trial1: string, trial2: string) {
    let sub = this.node.getElementsByClassName("modules")[0];

    json("Modules", sub, DiffInfoWidget.dependency_url(trial1, trial2), (data: DiffModuleData) => {
  ...
  loadFileAccess(trial1: string, trial2: string) {
    ...
    json("File Accesses", sub, DiffInfoWidget.accesses_url(trial1, trial2), (data: DiffAccessData) => {
      ...
      if ((data.fa_added.length > 0) ||
        ...
        this.createFold(accesses, "Modules");
  ...
  private info(main: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, label: string, value1: any, value2: any) {
    if (value1 == value2) {
      let attr = main.append("span")
        .classed("attr", true);
      attr.append("span")
  ...
  private mod_li(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, mod: DependencyData) {
  ...
  private env_field(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, env: EnvironmentItemData) {
    element.append("span")
      .classed("key", true)
      .text(env.name);
  ...
  private env_li(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, env: EnvironmentItemData) {
    this.env_field(
      element.append("li")
        .classed(cls, true),
      env
  ...
  private env_cli(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, rem: EnvironmentItemData, add: EnvironmentItemData) {
    let li = element.append("li");
    this.env_field(
      li.append("div")
        .classed("dbefore", true),
  ...
  private fa_field(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, title: string, value: string) {
    element.append("div")
      .classed(cls, true)
      .attr("title", title)
      .text(value);
  ...
  private fa_li(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, fa: FileAccessData) {
    let li = element.append("li")
      .classed(cls, true);
  ...
  private fa_cfield(element: d3_Selection<d3_BaseType, {}, HTMLElement, any>, cls: string, title: string, v1: string, v2: string) {
    if (v1 == v2) {
