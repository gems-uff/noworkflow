...
import {
  HistoryGraphData, HistoryNodeData, HistoryEdgeData
} from './structure';

export
interface HistoryConfig {
  customSelectNode?: (g: HistoryGraph, d: VisibleNode) => boolean;
  customCtrlClick?: (g: HistoryGraph, d: VisibleNode) => boolean;
  customSize?: (g: HistoryGraph) => number[];
  customForm?: (g: HistoryGraph, form: d3_Selection<d3_BaseType, {}, HTMLElement, any>) => void;
...
class HistoryGraph {
  ...
  hintElement: d3_Selection<d3_BaseType, {}, HTMLElement, any>;

  nodes: VisibleNode[] = [];
  versionNodes: VisibleNode[] = [];
  edges: VisibleEdge[] = [];
  maxX: number = 0;
  maxY: number = 0;
  maxId: number = 0;
  ...
  constructor(graphId:string, div: any, config: HistoryConfig = {}) {
    this.i = 0;
    var defaultConfig: HistoryConfig = {
      customSelectNode: (g: HistoryGraph, d: VisibleNode) => null,
      customCtrlClick: (g: HistoryGraph, d: VisibleNode) => null,
      customForm: (g: HistoryGraph, form: d3_Selection<d3_BaseType, {}, HTMLElement, any>) => null,
  ...
  load(data: HistoryGraphData): VisibleNode[] {
    let
      nodes: VisibleNode[] = [],
      otherNodes: VisibleNode[] = [],
      edges: VisibleEdge[] = [],
      spacing = this.config.spacing,
      margin = this.config.margin;
    let spacing2 = 2 * spacing,
      spacing4 = 4 * spacing,
      end = this.config.width - margin,
      start = margin,
      max = 0,
      id = 0,
    ...
    for (var i = 0; i < data.edges.length; i++) {
      ...
      edges.push(edge as VisibleEdge);
    }
  ...
  selectNode(node: VisibleNode): void {
  ...
  private showTooltip(d: VisibleNode) {
  ...
  private unselectNode(): void {
    let unselected = this.g.selectAll('g.node').filter(
      (cd: VisibleNode) => (cd.id === this.state.selectedNode.id)
    ).select('rect')
  ...
  private nodeMouseDown(d3node: d3_Selection<d3_BaseType, {}, HTMLElement, any>, d: VisibleNode): void {
    d3_event.stopPropagation();
    this.state.mouseDownNode = d;
    this.closeTooltip();
  }

  private nodeMouseUp(d3node: d3_Selection<d3_BaseType, {}, HTMLElement, any>, d: VisibleNode): void {
    d3_event.stopPropagation();
  ...
  private updateVersionNodes(nodes: any) {
    ...
    var nodeEnter = nodes.enter().append("g")
      ...
      .attr("attr-trialid", (d: VisibleNode) => d.title)
    ...
    nodeEnter.append('rect')
      ...
      .attr('width', (d: VisibleNode) => 2 * d.radius)
      .attr('height', (d: VisibleNode) => 2 * d.radius)
      .attr('rx', 0)
      .attr('ry', 0)
      //.attr('r', )
      .attr("stroke", "#000")
    ...
    nodeEnter.append('text')
      ...
      .attr('x', (d: VisibleNode) => d.radius)
      .attr('y', (d: VisibleNode) => d.radius + 4)
      .attr('stroke', '#000')
      .attr('text-anchor', 'middle')
      //.attr('font-weight', 'bold')
      .attr("transform", (d: VisibleNode) => {
        return "translate(" + d.x + "," + d.y + ")";
      }).text((d: VisibleNode) =>  d.display);
  ...
  private updateNodes(nodes: any) {
    ...
    nodeEnter.append('rect')
      ...
      .attr('cursor', 'pointer')
      .attr('title', (d: VisibleNode) => d.info.display)
      .attr('width', (d: VisibleNode) => 2 * d.radius)
      .attr('height', (d: VisibleNode) => 2 * d.radius)
      .attr('rx', (d: VisibleNode) => 2 * d.radius)
      .attr('ry', (d: VisibleNode) => 2 * d.radius)
      //.attr('r', )
      .attr("stroke", "#000")
      .attr("stroke-width", "2.5px")
      .attr("fill", function (d: VisibleNode) {
      ...
      .attr("stroke", function(d: VisibleNode) {
        return (d3_select(this).classed('selected')) ? 'rgb(200, 238, 241)' : "#000";
      })
      .attr("stroke-width", "2.5px")
      .on('mousedown', function(d: VisibleNode) {
        self.nodeMouseDown(d3_select(this), d);
      }).on('mouseup', function (d: VisibleNode) {
    ...
    nodeEnter.append('text')
      ...
      .attr("transform", (d: VisibleNode) => {
        return "translate(" + d.x + "," + d.y + ")";
      }).text((d: VisibleNode) =>  d.gradient? "" : d.display);
