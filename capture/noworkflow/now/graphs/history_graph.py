...
class HistoryGraph(Graph):
    ...
    def _preprocess_trials(self, trials_query):
        """Preprocess trials
        ...
        Arguments:
        trials_query -- trial query or trial list to be preprocessed
        """
        trials = []
        ids = []
        tmap = {}

        for trial in trials_query:
            trial.level = 0
            trial.tooltip = """
                <b>{0.script}</b><br>
                {status}<br>
    ...
    def _prepare_history_graph(self, trials, ids):
    ...
    def _edges(self, graph, nodes, script_order=None):
    ...
    def _create_edges(self, graph, nodes, id_map):
    ...
    def _set_trials_level(self, tmap, scripts, order, children, actual_graph):
        """Adjust levels of trials according to their scrip and branchs
    ...
    def history_data(self):
        ...
        trials, ids, tmap = self._preprocess_trials(
            self.history.trials.order_by(Trial.start.desc()).limit(MAX_TRIALS)
        )
        graph, nodes, id_map, scripts = self._prepare_history_graph(
            trials, ids
        )
        edges, order, children, actual_graph = self._create_edges(
            graph, nodes, id_map
        )
        self._set_trials_level(
